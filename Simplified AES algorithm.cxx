...
void mixColumns(vector<vector<uint8_t>>& state) {
    vector<vector<uint8_t>> temp(Nb, vector<uint8_t>(Nb));
    for (int i = 0; i < Nb; ++i) {
        for (int j = 0; j < Nb; ++j) {
            temp[j][i] = gmul(mixColMatrix[j][0], state[0][i]) ^
                         gmul(mixColMatrix[j][1], state[1][i]) ^
                         gmul(mixColMatrix[j][2], state[2][i]) ^
                         gmul(mixColMatrix[j][3], state[3][i]);
        }
    }
    state = temp;
}
...
int main() {
    ...
    cout << "Enter plaintext (32 hex chars): ";
for (int i = 0; i < 16; ++i) {
    cin >> hex >> state[i % 4][i / 4];
}
